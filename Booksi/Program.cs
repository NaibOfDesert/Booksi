 ...
 * 🐳 Docker Compose:
 *   - Use the "DefaultConnection" connection string.
 *   - Typically used when running the app in containers.

 * ▶️ dotnet run (local development):
 *   - Use the "LocalhostConnection" connection string.
 *   - For running the app directly on the host machine (e.g. Mac/Windows).

 * 🏗️ Creating migrations:
 *   - Run from the `../Booksi.DataAccess/` directory:
 *     dotnet ef migrations add Initial --startup-project ../Booksi/

 * 🛠️ Updating the database:
 *   - Run from the `../Booksi/` directory:
 *     dotnet ef database update
 *   - Make sure "LocalhostConnection" is active in your appsettings/environment.
 */
